## 🚀 About Me

- **Handle Name**: kimulaco
- **Gender**: He/Him
- **Location**: Japan
- **Role**: Web Developer
- **Contact**:
  - [X - kimulaco](https://x.com/kimulaco)
- **Web Site**:
  - [@kimulaco/blog](https://blog.kimulaco.dev)

## 🛠️ Tech Stack

### **Languages**

![HTML, CSS, Sass, Less, JavaScript, TypeScript, Node.js, PHP, Go, Rust](https://skillicons.dev/icons?i=html,css,sass,less,js,ts,nodejs,php,go,rust)

### **Frameworks & Libraries**

![Vue, Nuxt.js, React, Next.js, Astro, Tailwind, Vite, Vitest, Gulp](https://skillicons.dev/icons?i=vue,nuxtjs,react,nextjs,astro,tailwind,vite,vitest,gulp)

### **DB**

![MySQL, Redis, SQLite](https://skillicons.dev/icons?i=mysql,redis,sqlite)

### **DevOps & Infrastructure**

![AWS, Vercel, Netlify, Docker, GitHub, Github Actions, GitLab, Sentry, WordPress](https://skillicons.dev/icons?i=aws,vercel,netlify,docker,github,githubactions,gitlab,sentry,wordpress)

### **Development Tools**

![Visual Studio Code, Git, Figma](https://skillicons.dev/icons?i=vscode,git,figma)
